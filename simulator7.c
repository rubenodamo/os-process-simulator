...
int main() {
    // thread handles
    pthread_t generatorThread, simulatorThread, terminatorThread, boosterThread, ioThread;
    int i;
...
void * simulator() {
    ...
    while(1) {
        ...
        // else if the process is blocked, add it to the I/O queue
            ...
            printsf("SIMULATOR - CPU 0 - I/O BLOCKED: [PID = %d, Priority = %d, Device = %d]\n",
                cp->iPID, cp->iPriority, cp->iDeviceID);
